...
/* Get Committee details for reporting */
CREATE OR REPLACE PROCEDURE GET_COMMITTEE_DETAILS (
    p_user_id       IN  NUMBER,
    p_result_cursor OUT SYS_REFCURSOR
)
IS
BEGIN
    OPEN p_result_cursor FOR
        SELECT c.COMMITTEE_NAME, c.COMMITTEE_CODE, c.EMAIL, c.PHONE
        FROM ORGANIZING_COMMITTEE c
        JOIN USER_ROLE_LINK url ON c.COMMITTEE_ID = url.COMMITTEE_ID
        WHERE url.USER_ID = p_user_id AND url.ROLE_CODE = 'ORGANIZING_COMMITTEE';
END GET_COMMITTEE_DETAILS;
/
